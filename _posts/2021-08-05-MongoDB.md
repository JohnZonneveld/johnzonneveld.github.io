...
For example in a webstore solution, you still may have a customer collection and a product collection. The order collection however will contain all necesary information of both to be able to process the order. In other words,during an order lookup you can simply look in the order collection to find out what product the customer ordered but also get access to the shipping address. In a big order, product and customer base this will be faster than doing a order lookup in the order table, obtain both the customer.id and the product.id, then go to those tables and do a lookup to access the information needed.
