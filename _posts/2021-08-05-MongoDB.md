...
The command below will run in production, it will start our Node.js session and points to the server.js file (change if needed) that is our entry point of the application.
...
To create our web server we need to start express. This can be done with just a few commands.
{% highlight javascript %}
const app = express()
const PORT = 5000
...
MONGO_URI=mongodb://localhost:27017/household-book-mern // Path to our database
{% endhighlight %}
